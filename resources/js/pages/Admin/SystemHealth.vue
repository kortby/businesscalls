...
const mascotState = computed(() => {
    ...
    if (props.webhookRecoveryRate < 95.0 || props.averageDatabaseLockLatency > 20.0) {
        return 3;
    }
    ...
    <div class="min-h-screen bg-[#F0FDF4] p-6 text-slate-900 dark:bg-slate-950 dark:text-slate-100">
        ...
        >
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h1 class="text-2xl font-black tracking-tight uppercase sm:text-3xl">System Health</h1>
            ...
            <div class="flex gap-3">
                <div class="inline-flex items-center rounded-2xl border-4 border-slate-900 bg-amber-400 px-4 py-2 text-sm font-black uppercase dark:border-slate-100 dark:text-slate-950">
                    🔒 Gateway Active
                </div>
            </div>
        </header>
        ...
        <div class="grid grid-cols-1 gap-8 lg:grid-cols-3">
            ...
            <div class="flex flex-col gap-6 lg:col-span-1">
                ...
                >
                    ...
                    <div class="mt-2 space-y-2">
                        <div class="text-lg font-black tracking-tight uppercase">
                            <span v-if="mascotState === 2" class="text-emerald-600 dark:text-emerald-400">System Healthy 🎉</span>
                            <span v-else class="text-rose-600 dark:text-rose-400">Resilience Warning ⚠️</span>
                        </div>
                        <p class="px-4 text-xs font-bold text-slate-500 dark:text-slate-400">
                            <span v-if="mascotState === 2">
                                Webhook delivery queues are deduplicated, and database transactional lock latency is running optimal.
                            </span>
                            <span v-else>
                                High latency or packet loss detected. Middleware is protecting queues, but verification checks are advised.
                            </span>
                        </p>
                    </div>
                </div>
                ...
                >
                    ...
                    <div class="flex flex-col items-center justify-center rounded-2xl border-4 border-slate-900 bg-indigo-50 p-4 text-center dark:border-slate-100 dark:bg-indigo-950/30">
                        <span class="font-mono text-lg font-black text-indigo-600 dark:text-indigo-400">
                            &Phi;<sub>recovery</sub> = (1 - (F - R) / T) &times; 100%
                        </span>
                    </div>
                    <div class="mt-4 space-y-2 text-xs font-bold text-slate-500 dark:text-slate-400">
                        <p>Where:</p>
                        <ul class="list-disc pl-4 space-y-1">
                            <li><span class="font-black text-slate-700 dark:text-slate-200">T:</span> Total incoming webhook events</li>
                            <li><span class="font-black text-slate-700 dark:text-slate-200">F:</span> Total recorded network/delivery failures</li>
                            <li><span class="font-black text-slate-700 dark:text-slate-200">R:</span> Recovered webhook retry transactions</li>
                        </ul>
                    </div>
                </div>
            </div>
            ...
            <div class="flex flex-col gap-6 lg:col-span-2">
                ...
                <div class="grid grid-cols-1 gap-6 sm:grid-cols-3">
                    ...
                    <div
                        class="rounded-3xl border-4 border-slate-900 bg-white p-6 shadow-[4px_4px_0px_0px_rgba(15,23,42,1)] dark:border-slate-100 dark:bg-slate-900 dark:shadow-[4px_4px_0px_0px_rgba(255,255,255,1)]"
                    >
                        <h4 class="text-xs font-black tracking-widest text-slate-500 uppercase dark:text-slate-400">
                            Recovery Rate
                        </h4>
                        <div class="mt-4 flex items-baseline gap-2">
                            <span class="text-4xl font-black tracking-tight text-emerald-500">
                                {{ webhookRecoveryRate }}%
                            </span>
                        </div>
                        <div class="mt-4 h-6 w-full overflow-hidden rounded-full border-4 border-slate-900 bg-slate-100 dark:border-slate-100 dark:bg-slate-800">
                            <div
                    ...
                    >
                        ...
                        <div class="mt-4 flex items-baseline gap-2">
                            <span
                                ...
                                :class="[averageDatabaseLockLatency > 20.0 ? 'text-rose-500' : 'text-indigo-500']"
                            >
                                {{ averageDatabaseLockLatency }}ms
                            </span>
                        </div>
                        <div class="mt-2 flex items-center gap-1 text-xs font-bold text-slate-500 dark:text-slate-400">
                    ...
                    >
                        ...
                        <div class="mt-4 flex items-baseline gap-2">
                            <span class="text-4xl font-black tracking-tight text-amber-500">
                ...
                >
                    ...
                    <div class="flex-1 overflow-x-auto">
                        <table class="w-full text-left border-collapse">
                            <thead>
                                <tr class="border-b-4 border-slate-900 text-xs font-black uppercase text-slate-500 dark:border-slate-100 dark:text-slate-400">
                            ...
                            <tbody>
                                <tr
                                    v-for="(event, idx) in callStore.recentWebhookEvents"
                                    :key="idx"
                                ...
                                >
                                    <td class="py-3 px-4 text-slate-500 font-mono text-xs dark:text-slate-400">
                                        {{ formatTime(event.timestamp) }}
                                    </td>
                                    <td class="py-3 px-4 font-black">
                                        {{ event.event }}
                                    </td>
                                    <td class="py-3 px-4 text-slate-600 font-mono text-xs dark:text-slate-300">
                                        {{ event.event_id }}
                                    </td>
                                    <td class="py-3 px-4">
                                        <span
                                            v-if="event.is_duplicate"
                                            class="inline-flex items-center rounded-full border-2 border-slate-900 bg-amber-400 px-2.5 py-0.5 text-xs font-black uppercase text-slate-900"
                                        >
                                            Duplicate / Blocked
                                        </span>
                                        <span
                                            v-else
                                            class="inline-flex items-center rounded-full border-2 border-slate-900 bg-emerald-500 px-2.5 py-0.5 text-xs font-black uppercase text-white"
                                        >
                                            Unique / Processed
                                        </span>
                                    </td>
                                </tr>
                                <tr v-if="callStore.recentWebhookEvents.length === 0">
                                    <td colspan="4" class="py-8 text-center text-sm font-bold text-slate-400 uppercase">
