...
const computedPhiRoi = computed(() => {
    return props.subscriptionCost > 0
        ? computedSavings.value / props.subscriptionCost
        : 1.0;
});
...
const simulateGoalMet = () => {
    ...
    localRevenueRescued.value += 150.0;
};
    ...
    >
        ...
        >
            ...
            <div class="flex gap-3">
                <span
                    class="inline-flex items-center rounded-2xl border-4 border-slate-900 bg-[#3B82F6] px-4 py-2 text-sm font-black text-white uppercase dark:border-slate-100"
                >
        ...
        <div class="mb-8 grid grid-cols-1 gap-6 md:grid-cols-4">
            ...
            >
                <div
                    class="rounded-xl border-4 border-slate-900 bg-amber-400 p-2.5 text-slate-950"
                >
            ...
            <!-- Total Savings -->
            <div
                class="flex items-center gap-4 rounded-2xl border-4 border-slate-900 bg-emerald-500 p-4 text-white shadow-[4px_4px_0px_0px_rgba(15,23,42,1)] dark:border-slate-100 dark:shadow-[4px_4px_0px_0px_rgba(255,255,255,1)]"
            >
                <div
                    class="rounded-xl border-4 border-white bg-white p-2.5 text-emerald-600"
                >
        ...
        <div class="grid grid-cols-1 gap-8 lg:grid-cols-3">
            ...
            <div class="flex flex-col gap-8 lg:col-span-2">
                ...
                >
                    ...
                    <!-- ROI Index Meter -->
                    <div
                        class="mb-8 rounded-2xl border-4 border-slate-900 bg-[#F8FAFC] p-6 dark:border-slate-100 dark:bg-slate-950"
                    >
                        <div class="mb-4 flex items-center justify-between">
                            ...
                            <span
                                class="rounded-xl border-4 border-slate-900 bg-emerald-400 px-3 py-1.5 text-xs font-black text-slate-950 uppercase dark:border-slate-100"
                            >
                                {{
                                    computedPhiRoi >= 1.0
                                        ? 'Profitable'
                        ...
                        <!-- Progress Bar Meter -->
                        <div
                            class="h-6 w-full overflow-hidden rounded-full border-4 border-slate-900 bg-slate-200 dark:border-slate-100"
                        >
                            <div
                                class="h-full bg-emerald-500 transition-all duration-500 ease-out"
                                :style="{
                                    width: `${Math.min(100, (computedPhiRoi / 5.0) * 100)}%`,
                                }"
                            ></div>
                        </div>
                        <div
                            class="mt-2 flex justify-between text-xs font-bold text-slate-400"
                        >
                    ...
                    <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                        <div
                            class="rounded-2xl border-4 border-slate-900 p-4 dark:border-slate-100"
                        >
                            <div class="mb-2 flex items-center justify-between">
                                <span class="text-sm font-black uppercase"
                                    >Daily Revenue Goal</span
                                >
                                <Check
                                    v-if="computedSavings >= targetRevenue"
                            ...
                            <p class="mt-1 text-xs text-slate-500">
                                Goal matches targeted after-hours intake
                                revenue.
                            </p>
                        </div>
                        ...
                        >
                            <span
                                class="mb-2 block text-sm font-black uppercase"
                                >SaaS Pricing Factor</span
                            >
                            <div class="text-2xl font-black">
                                ${{ subscriptionCost }}/mo
                            </div>
                            <p class="mt-1 text-xs text-slate-500">
                                Monthly cost based on active plan level.
                            </p>
                        </div>
                    </div>
                </div>
                ...
                >
                    ...
                    <div class="grid grid-cols-1 gap-4 sm:grid-cols-4">
                        <button
                            @click="simulateRouteOptimization"
                            class="duo-btn duo-btn-info flex items-center justify-center gap-1 text-xs"
                        >
                            <Play class="h-3.5 w-3.5 stroke-[3]" />
                            Optimize Route
                        </button>
                        <button
                            @click="simulateGoalMet"
                            class="duo-btn duo-btn-success flex items-center justify-center gap-1 text-xs"
                        >
                            <Check class="h-3.5 w-3.5 stroke-[3]" />
                            Goal Met
                        </button>
                        <button
                            @click="simulateBillingError"
                            class="duo-btn duo-btn-warning flex items-center justify-center gap-1 text-xs"
                            style="
                        ...
                        <button
                            ...
                            class="duo-btn duo-btn-muted flex items-center justify-center gap-1 text-xs"
                        >
