...
const technicianRosterSim = ref<'Mapped' | 'Empty'>(
    props.technicianRosterStatus,
);
const voiceAiPromptsSim = ref<'Programmed' | 'Default'>(
    props.voiceAiPromptsStatus,
);
...
const startCheckout = async (plan: 'pro' | 'enterprise') => {
    ...
    try {
        ...
        if (response.ok && data.url) {
            ...
            upgradeMessage.value =
                data.error || 'Failed to start billing checkout.';
...
const triggerPaymentFailure = () => {
    paymentFailedSim.value = true;
    upgradeMessage.value =
        '⚠️ Simulated Payment failure recorded. Mascot state locked to error.';
};
...
<template>
    ...
    >
        ...
        <div
            class="relative overflow-hidden rounded-3xl border-4 border-slate-800 bg-slate-900 p-6 shadow-[6px_6px_0px_0px_rgba(16,185,129,0.3)] sm:p-8"
        >
            <div
            ...
            >
                <div>
                    ...
                    >
                        Configure SaaS billing plans, technician skills
                        profiles, and voice agent system prompts.
                    </p>
                </div>

                <!-- Active plan tier widget -->
                <div
                    class="flex min-w-[280px] flex-col gap-1.5 rounded-2xl border-4 border-slate-800 bg-slate-950 p-4"
                >
                    <div
                    ...
                    >
                        ...
                        <span
                            class="font-black tracking-wider text-emerald-500 capitalize"
                            >{{ tenant.plan || 'Free' }}</span
                        >
                    </div>

                    <div
                        class="bg-slate-850 mt-1 h-4 w-full overflow-hidden rounded-full border-2 border-slate-700"
                    >
                        <div
                    ...
                    <div
                        class="mt-1 flex items-center justify-between text-[11px] font-bold text-slate-500"
                    >
                        <span>Checkout Mode: Stripe</span>
                        <span
                            v-if="isSubscribedSim"
                            class="font-black text-emerald-400"
                            >Active Subscriber</span
                        >
                        <span v-else class="text-slate-400"
                            >Trial/Basic limits</span
                        >
                    </div>
                </div>
            </div>
                            ...
                            <tbody class="divide-slate-850 divide-y">
                                ...
                                >
                                    ...
                                    <td class="p-3 text-right">
                                        <span
                                            class="inline-flex items-center gap-1.5 rounded-lg border-2 px-2.5 py-0.5 text-[9px] font-black uppercase"
                                            :class="[
                                                isSubscribedSim
                                                    ? 'border-emerald-800 bg-emerald-950/60 text-emerald-400'
                                                    : 'border-slate-750 bg-slate-900 text-slate-400',
                                            ]"
                                        >
                                            <CheckCircle2
                                                v-if="isSubscribedSim"
                ...
                >
                    ...
                    <div class="grid grid-cols-1 gap-4 md:grid-cols-3">
                            ...
                            @click="
                                technicianRosterSim =
                                    technicianRosterSim === 'Mapped'
                                        ...
                                        : 'Mapped'
                            "
                            class="rounded-2xl border-4 border-slate-700 bg-slate-800 p-3 text-xs font-black text-slate-300 uppercase transition-all hover:bg-slate-700"
                        >
                        ...
                        <button
                            ...
                            class="flex items-center justify-center gap-1.5 rounded-2xl border-4 border-slate-700 bg-slate-800 p-3 text-xs font-black text-slate-300 uppercase transition-all hover:bg-slate-700"
                        >
            ...
            <div class="flex flex-col gap-8">
                ...
                >
                    ...
                    <p class="text-xs leading-relaxed font-bold text-slate-400">
                        Instantly deploy premium, dedicated dispatch routes by
                        subscribing to a professional platform plan.
                    </p>

                    <div
                        v-if="upgradeMessage"
                        class="rounded-xl border-2 border-slate-800 bg-slate-950 p-3 text-xs font-bold text-amber-400"
                    >
                        {{ upgradeMessage }}
                    </div>
                    ...
                    <div class="flex flex-col gap-3">
                        <button
                            ...
                            class="inline-flex w-full items-center justify-center gap-2 rounded-2xl border-4 border-emerald-700 bg-emerald-500 py-3 text-xs font-black tracking-wider text-white uppercase transition-all hover:bg-emerald-400 active:translate-y-[2px] disabled:opacity-50"
                        >
                        ...
                        <button
                            ...
                            class="inline-flex w-full items-center justify-center gap-2 rounded-2xl border-4 border-rose-900 bg-rose-950 py-3 text-xs font-black tracking-wider text-rose-400 uppercase transition-all hover:bg-rose-900/60 active:translate-y-[2px]"
                        >
