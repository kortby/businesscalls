...
const logs = ref<Array<{ id: string; time: string; msg: string; type: 'info' | 'success' | 'warning' }>>([]);
...
onMounted(() => {
    ...
    if (tenantId) {
        ...
        useEcho(`tenant.${tenantId}`, 'dispatch.updated', (payload: any) => {
            addLog(`Websocket update: ${payload.message || 'Dispatch status changed'}`, 'info');
            
            // Set mascot state to Victory for booking confirmed
            if (payload.type === 'booking_created' || payload.type === 'booking_confirmed') {
                mascotState.value = 2;
                setTimeout(() => {
            ...
            if (payload.booking) {
                const idx = localBookings.value.findIndex(b => b.id === payload.booking.id);
                if (idx !== -1) {
        ...
        useEcho(`tenant.${tenantId}`, 'CallStarted', (payload: any) => {
            addLog(`Incoming call started from ${payload.customer_phone || 'Customer'}`, 'warning');
        ...
        useEcho(`tenant.${tenantId}`, 'CallEnded', (payload: any) => {
            ...
            const existing = localCalls.value.find(c => c.call_id === payload.call_id);
            if (existing) {
...
const simulateBookingConfirmation = () => {
    ...
    const randomTech = localTechnicians.value[Math.floor(Math.random() * localTechnicians.value.length)] || null;
    ...
    if (randomTech) {
        const tIdx = localTechnicians.value.findIndex(t => t.id === randomTech.id);
        if (tIdx !== -1) {
...
const simulateTechnicianStatusUpdate = () => {
    ...
    const randomTech = localTechnicians.value[Math.floor(Math.random() * localTechnicians.value.length)];
    const nextStatus = statuses[Math.floor(Math.random() * statuses.length)];
    
    randomTech.status = nextStatus;
    addLog(`Simulation: Technician ${randomTech.first_name} is now ${nextStatus.replace('_', ' ')}.`, 'info');
    ...
    <div class="min-h-screen bg-[#F0FDF4] p-6 text-slate-900 dark:bg-slate-950 dark:text-slate-100">
        ...
        >
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h1 class="text-2xl font-black tracking-tight uppercase sm:text-3xl">Live Dispatch Board</h1>
            ...
            <div class="flex gap-3">
                <div class="inline-flex items-center rounded-2xl border-4 border-slate-900 bg-emerald-500 px-4 py-2 text-sm font-black text-white uppercase dark:border-slate-100">
                    <span class="mr-2 h-2.5 w-2.5 animate-ping rounded-full bg-white"></span>
                    Live Streaming
                </div>
            </div>
        </header>
        ...
        <div class="mb-8 grid grid-cols-1 gap-6 sm:grid-cols-3">
            <div class="flex items-center gap-4 rounded-2xl border-4 border-slate-900 bg-white p-4 shadow-[4px_4px_0px_0px_rgba(15,23,42,1)] dark:border-slate-100 dark:bg-slate-900 dark:shadow-[4px_4px_0px_0px_rgba(255,255,255,1)]">
                ...
                <div>
                    <div class="text-2xl font-black">{{ localBookings.length }}</div>
                    <div class="text-xs font-bold text-slate-500 uppercase dark:text-slate-400">Booked Jobs</div>
                </div>
            </div>
            ...
            <div class="flex items-center gap-4 rounded-2xl border-4 border-slate-900 bg-white p-4 shadow-[4px_4px_0px_0px_rgba(15,23,42,1)] dark:border-slate-100 dark:bg-slate-900 dark:shadow-[4px_4px_0px_0px_rgba(255,255,255,1)]">
                ...
                <div>
                    <div class="text-2xl font-black">
                        {{ localCalls.filter(c => c.status === 'ongoing').length }}
                    </div>
                    <div class="text-xs font-bold text-slate-500 uppercase dark:text-slate-400">Ongoing Calls</div>
                </div>
            </div>

            <div class="flex items-center gap-4 rounded-2xl border-4 border-slate-900 bg-white p-4 shadow-[4px_4px_0px_0px_rgba(15,23,42,1)] dark:border-slate-100 dark:bg-slate-900 dark:shadow-[4px_4px_0px_0px_rgba(255,255,255,1)]">
                <div class="rounded-xl border-4 border-slate-900 bg-yellow-400 p-2.5 text-slate-950">
                    <User class="h-6 w-6 stroke-[3]" />
                </div>
                <div>
                    <div class="text-2xl font-black">{{ localTechnicians.length }}</div>
                    <div class="text-xs font-bold text-slate-500 uppercase dark:text-slate-400">Technicians</div>
                </div>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 gap-8 lg:grid-cols-3">
            ...
            <div class="flex flex-col gap-8 lg:col-span-2">
                <!-- Live SVG Map Box -->
                <div class="relative flex flex-col rounded-3xl border-4 border-slate-900 bg-white p-4 shadow-[4px_4px_0px_0px_rgba(15,23,42,1)] dark:border-slate-100 dark:bg-slate-900 dark:shadow-[4px_4px_0px_0px_rgba(255,255,255,1)]">
                    <div class="mb-4 flex items-center justify-between">
                        <h2 class="text-lg font-black uppercase tracking-tight">Interactive Dispatch Radar</h2>
                        <span class="text-xs font-bold text-slate-400 uppercase">Map Coordinates (Deterministic Grid)</span>
                    </div>

                    <!-- SVG Map Canvas -->
                    <div class="relative overflow-hidden rounded-2xl border-4 border-slate-900 bg-emerald-50/50 dark:bg-slate-950">
                        <svg viewBox="0 0 500 350" class="w-full h-auto max-h-[380px]">
                            <!-- Grid Overlay Patterns -->
                            <defs>
                                <pattern id="mapGrid" width="25" height="25" patternUnits="userSpaceOnUse">
                                    <path d="M 25 0 L 0 0 0 25" fill="none" stroke="currentColor" class="text-slate-900/10 dark:text-slate-100/10" stroke-width="1.5" />
                                </pattern>
                            </defs>
                            <rect width="500" height="350" fill="url(#mapGrid)" />

                            <!-- Dynamic Street Map Outline -->
                            <g stroke="currentColor" class="text-slate-900/15 dark:text-slate-100/15" stroke-width="4" stroke-dasharray="3 3">
                            ...
                            <g v-for="booking in localBookings.filter(b => b.employee_id && b.status !== 'completed')" :key="`vector-${booking.id}`">
                                <line
                                    :x1="getCoords('tech', booking.employee_id).x"
                                    :y1="getCoords('tech', booking.employee_id).y"
                            ...
                            <!-- Bookings Pins -->
                            <g v-for="booking in localBookings" :key="`booking-pin-${booking.id}`"
                               @click="selectedNode = { type: 'booking', data: booking }"
                               class="cursor-pointer group">
                                ...
                                <circle
                                    ...
                                    :cy="getCoords('booking', booking.id).y"
                                    r="8"
                                    :fill="booking.status === 'completed' ? '#10B981' : '#3B82F6'"
                                    stroke="#0f172a"
                                    stroke-width="2.5"
                                    class="transition-all duration-300 transform group-hover:scale-125"
                                />
                                <!-- Mini marker tag -->
                                <text
                                    ...
                                    text-anchor="middle"
                                    fill="currentColor"
                                    class="bg-white px-1 py-0.5 rounded border border-slate-900 text-slate-800 animate-bounce"
                                >
                                    Job #{{ booking.id }}
                                </text>
                            </g>
                            ...
                            <g v-for="tech in localTechnicians" :key="`tech-pin-${tech.id}`"
                               @click="selectedNode = { type: 'tech', data: tech }"
                               class="cursor-pointer group">
                                <circle
                                    :cx="getCoords('tech', tech.id).x"
                                    :cy="getCoords('tech', tech.id).y"
                                    r="10"
                                    :fill="tech.status === 'en_route' ? '#F59E0B' : tech.status === 'on_site' ? '#EF4444' : '#10B981'"
                                    stroke="#0f172a"
                                    stroke-width="2.5"
                                    class="transition-all duration-300 transform group-hover:scale-125"
                                />
                    ...
                    <div class="mt-4 flex flex-wrap gap-4 border-t-2 border-slate-900/10 pt-4 text-xs font-bold dark:border-slate-100/10">
                        <div class="flex items-center gap-1.5">
                            <span class="h-3.5 w-3.5 rounded-full border-2 border-slate-900 bg-[#3B82F6]"></span>
                            <span>Pending Job</span>
                        </div>
                        <div class="flex items-center gap-1.5">
                            <span class="h-3.5 w-3.5 rounded-full border-2 border-slate-900 bg-[#10B981]"></span>
                            <span>Completed Job</span>
                        </div>
                        <div class="flex items-center gap-1.5">
                            <span class="h-3.5 w-3.5 rounded-full border-2 border-slate-900 bg-[#F59E0B]"></span>
                            <span>Tech: En Route</span>
                        </div>
                        <div class="flex items-center gap-1.5">
                            <span class="h-3.5 w-3.5 rounded-full border-2 border-slate-900 bg-[#EF4444]"></span>
                ...
                <div v-if="selectedNode"
                     class="rounded-3xl border-4 border-slate-900 bg-white p-6 shadow-[4px_4px_0px_0px_rgba(15,23,42,1)] dark:border-slate-100 dark:bg-slate-900 dark:shadow-[4px_4px_0px_0px_rgba(255,255,255,1)] animate-in fade-in slide-in-from-bottom-2 duration-200">
                    <div class="flex items-center justify-between border-b-4 border-slate-900 pb-4 mb-4 dark:border-slate-100">
                        <div class="flex items-center gap-2">
                            <Info class="h-5 w-5 text-amber-500 stroke-[3]" />
                            <h3 class="text-md font-black uppercase tracking-tight">
                                {{ selectedNode.type === 'tech' ? 'Technician Details' : selectedNode.type === 'booking' ? 'Booking Details' : 'Active Call Details' }}
                            </h3>
                        </div>
                        <button @click="selectedNode = null" class="text-xs font-black uppercase border-2 border-slate-900 px-2.5 py-1 rounded-md hover:bg-slate-100 dark:hover:bg-slate-800">
                            Close
                        </button>
                    </div>

                    <!-- Technician Information -->
                    <div v-if="selectedNode.type === 'tech'" class="grid grid-cols-1 gap-4 sm:grid-cols-2 text-sm font-bold">
                        <div>
                            <div class="text-slate-400 uppercase text-xs">Name</div>
                            <div class="text-base font-black">{{ selectedNode.data.first_name }} {{ selectedNode.data.last_name }}</div>
                        </div>
                        <div>
                            <div class="text-slate-400 uppercase text-xs">Phone</div>
                            <div class="text-base">{{ selectedNode.data.phone }}</div>
                        </div>
                        <div>
                            <div class="text-slate-400 uppercase text-xs">Status</div>
                            <div class="inline-block px-2.5 py-0.5 rounded-lg border-2 border-slate-900 capitalize"
                                 :class="[selectedNode.data.status === 'en_route' ? 'bg-amber-400' : selectedNode.data.status === 'on_site' ? 'bg-rose-400' : 'bg-emerald-400']">
                                {{ selectedNode.data.status }}
                            </div>
                        </div>
                        <div>
                            <div class="text-slate-400 uppercase text-xs">Skills</div>
                            <div class="flex flex-wrap gap-1 mt-1">
                                <span v-for="skill in selectedNode.data.skills" :key="skill" class="border border-slate-900 px-1.5 py-0.5 rounded text-xs">
                                    {{ skill }}
                                </span>
                            </div>
                        </div>
                    ...
                    <!-- Booking Information -->
                    <div v-if="selectedNode.type === 'booking'" class="grid grid-cols-1 gap-4 sm:grid-cols-2 text-sm font-bold">
                        <div>
                            <div class="text-slate-400 uppercase text-xs">Job Details</div>
                            <div class="text-base font-black">{{ selectedNode.data.job_details }}</div>
                        </div>
                        <div>
                            <div class="text-slate-400 uppercase text-xs">Customer Phone</div>
                            <div class="text-base">{{ selectedNode.data.customer_phone }}</div>
                        </div>
                        <div>
                            <div class="text-slate-400 uppercase text-xs">Status</div>
                            <div class="inline-block px-2.5 py-0.5 rounded-lg border-2 border-slate-900 capitalize bg-blue-300">
                                {{ selectedNode.data.status }}
                            </div>
                        </div>
                        <div>
                            <div class="text-slate-400 uppercase text-xs">Assigned Technician</div>
                            <div class="text-base text-amber-500 font-black">
                                {{ selectedNode.data.employee ? `${selectedNode.data.employee.first_name} ${selectedNode.data.employee.last_name}` : 'Not Assigned' }}
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="flex flex-col gap-8">
                ...
                <div class="flex flex-col items-center justify-center rounded-3xl border-4 border-slate-900 bg-white p-6 shadow-[4px_4px_0px_0px_rgba(15,23,42,1)] dark:border-slate-100 dark:bg-slate-900 dark:shadow-[4px_4px_0px_0px_rgba(255,255,255,1)]">
                    ...
                    <div class="w-full rounded-2xl border-4 border-slate-900 bg-slate-50 p-4 font-bold text-xs dark:bg-slate-950 dark:border-slate-100">
                        <div class="flex justify-between mb-1">
                            <span>Mascot Speech:</span>
                            <span class="text-emerald-500">{{ callStore.isSpeaking ? 'ACTIVE SPEAKING' : 'SILENT' }}</span>
                        </div>
                        <div class="flex justify-between mb-1">
                        ...
                        <div class="flex justify-between">
                            ...
                            <span class="uppercase font-black text-amber-500">State {{ mascotState }}</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="rounded-3xl border-4 border-slate-900 bg-white p-6 shadow-[4px_4px_0px_0px_rgba(15,23,42,1)] dark:border-slate-100 dark:bg-slate-900 dark:shadow-[4px_4px_0px_0px_rgba(255,255,255,1)]">
                    ...
                    <div class="flex flex-col gap-3.5">
                        <button
                            ...
                            class="duo-btn duo-btn-warning w-full flex items-center justify-center gap-2"
                        >
                        ...
                        <button
                            ...
                            class="duo-btn duo-btn-success w-full flex items-center justify-center gap-2"
                        >
                        ...
                        <button
                            ...
                            class="duo-btn duo-btn-info w-full flex items-center justify-center gap-2"
                        >
                        ...
                        <button
                            ...
                            class="duo-btn duo-btn-muted w-full flex items-center justify-center gap-2"
                        >
                ...
                <div class="flex flex-col rounded-3xl border-4 border-slate-900 bg-white p-6 shadow-[4px_4px_0px_0px_rgba(15,23,42,1)] dark:border-slate-100 dark:bg-slate-900 dark:shadow-[4px_4px_0px_0px_rgba(255,255,255,1)]">
                    ...
                    <div class="flex flex-col gap-3 max-h-[160px] overflow-y-auto pr-1">
                        <div
                            v-for="log in logs"
                            :key="log.id"
                            class="flex items-start gap-2 text-xs font-bold border-b border-slate-900/10 pb-2 dark:border-slate-100/10 last:border-b-0"
                        >
                            <span class="text-slate-400">{{ log.time }}</span>
                            <span :class="[log.type === 'success' ? 'text-emerald-500' : log.type === 'warning' ? 'text-amber-500' : 'text-blue-500']">
                                {{ log.msg }}
                            </span>
                        </div>
                        <div v-if="logs.length === 0" class="text-center py-4 text-slate-400 text-xs">
