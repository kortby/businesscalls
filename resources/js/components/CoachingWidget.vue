...
const subscribeToCoaching = (callId: string) => {
    ...
    console.log(`[CoachingWidget] Subscribing to call whisper channel: tenant.${props.tenantId}.coaching.${callId}`);
    
    try {
        const channel = window.Echo.private(`tenant.${props.tenantId}.coaching.${callId}`)
        ...
        console.error('[CoachingWidget] Failed to subscribe to private coaching channel:', e);
    }
};

// Leave a specific call's coaching channel
const unsubscribeFromCoaching = (callId: string) => {
    const index = activeCallChannels.value.findIndex(c => c.callId === callId);
    if (index !== -1) {
        console.log(`[CoachingWidget] Leaving whisper channel for call: ${callId}`);
        try {
...
onMounted(() => {
    ...
    if (props.tenantId) {
        useEcho(`tenant.${props.tenantId}`, 'CallStarted', (payload: any) => {
            console.log('[CoachingWidget] CallStarted webhook received:', payload);
            
            const customerPhone = payload.customer_phone || payload.customerPhone;
            ...
            const hasBooking = props.bookings.some(b => {
                ...
                return bPhoneClean.includes(payloadPhoneClean) || payloadPhoneClean.includes(bPhoneClean);
            });
        ...
        useEcho(`tenant.${props.tenantId}`, 'CallEnded', (payload: any) => {
            console.log('[CoachingWidget] CallEnded webhook received:', payload);
            const callId = payload.call_id || payload.id;
            if (callId) {
...
<template>
    ...
    <div class="pointer-events-none fixed top-4 inset-x-4 z-[9999] flex flex-col items-center gap-3">
        <TransitionGroup name="slide-down">
            ...
            >
                <div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border-2 border-slate-900 bg-white dark:border-slate-100">
                    <MessageSquare class="h-5 w-5 text-amber-500 stroke-[3]" />
                </div>
                ...
                <div class="flex-1 text-slate-950">
                    <div class="flex items-center justify-between">
                        ...
                        <span class="text-[10px] font-black text-slate-500">{{ whisper.timestamp }}</span>
                    </div>
                    <p class="mt-1 text-sm font-extrabold leading-relaxed">
                        {{ whisper.instruction }}
                    </p>
                </div>
                ...
                <button
                    ...
                    class="shrink-0 rounded-lg border-2 border-slate-900 bg-white p-1 text-slate-700 hover:bg-slate-100 dark:border-slate-100 active:translate-y-0.5"
                >
